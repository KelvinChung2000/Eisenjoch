...
use crate::types::{ClockEdge, DelayT, IdString, IdStringPool, PortType, TimingPortClass};
use log::debug;
...
pub struct TimingAnalyser {
    ...
    arrival_times: FxHashMap<(CellId, IdString), DelayT>,
    /// Port required times (backward pass): (cell, port) -> required time.
    required_times: FxHashMap<(CellId, IdString), DelayT>,
    ...
    port_classes: FxHashMap<(CellId, IdString), TimingPortClass>,
    /// Clock domain assignment for register ports.
    /// (cell, port) -> ClockDomain
    port_domains: FxHashMap<(CellId, IdString), ClockDomain>,
}
...
impl TimingAnalyser {
    ...
    fn forward_propagation(&mut self, design: &Design, sorted_cells: &[CellId]) {
            ...
            for (port_name, port_info) in &cell.ports {
                ...
                match port_class {
                    TimingPortClass::RegisterOutput => {
                        ...
                        self.arrival_times
                            .insert((cell_idx, *port_name), clk_to_out);
                    }
        ...
        for &cell_idx in sorted_cells {
            ...
            for (port_name, port_info) in &cell.ports {
                ...
                if let Some(&driver_arrival) = self.arrival_times.get(&(driver_cell, driver_port)) {
                    // Estimate net delay.
                    let net_delay = self.estimate_net_delay(design, net_idx);
    ...
    fn backward_propagation(&mut self, design: &Design, sorted_cells: &[CellId]) {
            ...
            for (port_name, port_info) in &cell.ports {
                ...
                if port_info.net.is_some() {
                    ...
                    for user in &net.users {
                        ...
                        let user_cell = match user_cell {
                            Some(cell_idx) => cell_idx,
                            None => continue,
                        };
                        if let Some(&user_required) =
                            self.required_times.get(&(user_cell, user.port))
                        {
                            let net_delay = self.estimate_net_delay(design, net_idx);
    ...
    fn estimate_net_delay(&self, design: &Design, net_idx: NetId) -> DelayT {
        let net = design.net(net_idx);
        if !net.driver.is_connected() {
            return 0;
        }
        ...
        let driver_cell = design.cell(driver_cell_idx);
        if driver_cell.bel.is_none() {
            return 0;
        }
...
impl TimingAnalyser {
    ...
    /// Get arrival time for a (cell, port) pair (for testing).
    pub fn arrival_time(&self, cell: CellId, port: IdString) -> Option<DelayT> {
        self.arrival_times.get(&(cell, port)).copied()
    }

    /// Get required time for a (cell, port) pair (for testing).
    pub fn required_time(&self, cell: CellId, port: IdString) -> Option<DelayT> {
        self.required_times.get(&(cell, port)).copied()
    }

    /// Get port classification for a (cell, port) pair (for testing).
    pub fn port_class(&self, cell: CellId, port: IdString) -> Option<TimingPortClass> {
        self.port_classes.get(&(cell, port)).copied()
    }

    /// Get clock domain for a (cell, port) pair (for testing).
    pub fn port_domain(&self, cell: CellId, port: IdString) -> Option<&ClockDomain> {
        self.port_domains.get(&(cell, port))
    }
