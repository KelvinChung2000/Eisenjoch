...
use anderson::AndersonAccelerator;
use network::PipeNetwork;
use resistance::ResistanceModel;
...
pub fn place_opt_trans(ctx: &mut Context, cfg: &OptTransPlacerCfg) -> Result<(), PlacerError> {
    ...
    for iter in 0..cfg.max_iters {
        ...
        // c. Solve L*P = S via CG with Jacobi preconditioner.
        ...
        if is_report {
            ...
            eprintln!(
                "OptTrans iter {}: chpwl={:.0}, residual={:.3e}, cg_iters={}, max_util={:.2}, converged={}",
                iter, chpwl, resid, cg_result.iterations, max_util, cg_result.converged,
            );
        }
...
/// Compute HPWL from actual placed BEL positions.
fn compute_placed_hpwl(ctx: &Context) -> f64 {
    let mut hpwl = 0.0f64;
