//! Main placement loop for the optimal transport placer.
...
pub fn place_opt_trans(ctx: &mut Context, cfg: &OptTransPlacerCfg) -> Result<(), PlacerError> {
    ...
    for iter in 0..cfg.max_outer_iters {
        ...
        let overlap = if iter > 0 {
            Some(OptTransState::overlap_metrics_from_field(&pre_density))
        } else {
            None
        };
        ...
        let (_, max_rho, _) = overlap
            .unwrap_or_else(|| OptTransState::overlap_metrics_from_field(&pre_density));
    ...
    let (pre_overflow, pre_max_rho, _) =
        OptTransState::overlap_metrics_from_field(&pre_density);
    eprintln!(
        "Pre-legalization:  CHPWL={:.0}, overflow={:.1}%, maxρ={:.1}",
        pre_chpwl, pre_overflow * 100.0, pre_max_rho,
    );

    super::legalize::legalize_opt_trans(ctx, &state, cfg.lap_max_cells)?;
