...
fn main() {
    let db = ChipDb::load(Path::new("/home/kelvin/side-project/eisenjoch/chip_database/xc7_large.bin")).expect("load");
    ...
    for (n, c) in &by_name { println!("  {n} : {c}"); }
    ...
    for (sx, sy) in [(0i32, 183), (0, 152), (309, 119), (0, 98)] {
        ...
        println!("  ({sx},{sy}): type_idx={} name='{}'",
                 cdb.tile_type_index(t), cdb.tile_type_name(t));
    }

    // And tile types at sample destinations
    println!("\nFailing dest tiles:");
    for (sx, sy) in [(130i32, 118), (148, 102), (199, 90), (104, 105),
                     (148, 113), (189, 117), (131, 106), (310, 111)] {
        let t = sy * cdb.width() + sx;
        println!("  ({sx},{sy}): type_idx={} name='{}'",
                 cdb.tile_type_index(t), cdb.tile_type_name(t));
    }
