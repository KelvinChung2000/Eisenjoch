...
fn main() {
    ...
    let mut hits: Vec<(f64, i64, i64, String, (i32, i32), String, String, (i32, i32), String)> = Vec::new();

    for tt in 0..num_tile_types {
        let Some(rep_tile) = rep_of_type[tt] else { continue };
        ...
        for (pip_idx, pip) in pips.iter().enumerate() {
            ...
            cdb.node_wires_cb(dst_wire, |peer| {
                ...
                if s > max_span {
                    ...
                    peer_name = cdb.constid_str(nid).map(|s| s.to_string()).unwrap_or_default();
                }
            });
            if max_span < 30 { continue; }
            ...
            let src_name = if (src_wi as usize) < wires.len() {
                ...
                cdb.constid_str(nid).map(|s| s.to_string()).unwrap_or_default()
            } else { "?".into() };
            let dst_name = if (dst_wi as usize) < wires.len() {
                let nid: i32 = unsafe { read_packed!(wires[dst_wi as usize], name) };
                cdb.constid_str(nid).map(|s| s.to_string()).unwrap_or_default()
            } else { "?".into() };
            hits.push((rate, span, cost, tt_name.clone(), (rep_x, rep_y), src_name, dst_name, peer_xy, peer_name));
        }
    }
    ...
    let labels = ["30-49","50-69","70-99","100-129","130-159","160-199","200-249","250-299","300+","","",""];
    for (i, n) in bucket.iter().enumerate() {
        if *n > 0 { println!("  {}: {}", labels[i], n); }
    }
