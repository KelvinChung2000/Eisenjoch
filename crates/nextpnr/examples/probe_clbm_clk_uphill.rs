...
fn describe(chipdb: &ChipDb, wire: WireId) -> String {
    ...
    format!("{}({}:{}) [{},{}] type={} nid={:?} peers={}", wname, wire.tile(), wire.index(), x, y, wtype, nid, n_peers)
}
...
fn main() {
    ...
    let target_widx: i32 = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(1726);
    let max_layers: usize = std::env::args().nth(4).and_then(|s| s.parse().ok()).unwrap_or(6);
    ...
    while let Some((w, depth)) = frontier.pop_front() {
        ...
        db.node_wires_cb(w, |p| { if p != w { peers.push(p); } });
        ...
        if up_pips.len() > 20 {
            ...
            for &pidx in &up_pips {
                ...
                if seen.insert(src) { frontier.push_back((src, depth + 1)); }
            }
        }
        if peers.len() > 8 {
            println!("    {} node peers (sample: {}, {}, {})", peers.len(),
                describe(&db, peers[0]), describe(&db, peers[1]), describe(&db, peers[peers.len()-1]));
            // BFS only one representative peer at this depth
            if seen.insert(peers[0]) { frontier.push_back((peers[0], depth)); }
        } else {
            for &p in &peers {
                println!("    ~= {}", describe(&db, p));
                if seen.insert(p) { frontier.push_back((p, depth)); }
            }
        }
        let _ = nid;
    }
