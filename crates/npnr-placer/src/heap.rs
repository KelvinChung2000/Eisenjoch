...
use npnr_netlist::CellIdx;
use npnr_types::{BelId, PlaceStrength};
use rustc_hash::FxHashMap;

use crate::sa::PlacerError;
...
impl SparseSystem {
    ...
    fn add_connection(&mut self, i: usize, j: usize, weight: f64) {
    ...
    fn add_anchor(&mut self, i: usize, pos: f64, weight: f64) {
    ...
    fn solve(&self, x: &mut [f64], tolerance: f64, max_iters: usize) -> usize {
        ...
        conjugate_gradient(&self.diag, &self.off_diag, &self.rhs, x, tolerance, max_iters)
    }
}
...
struct HeapState {
    ...
    movable_cells: Vec<CellIdx>,
    /// Map from CellIdx to index in movable_cells.
    cell_to_idx: FxHashMap<CellIdx, usize>,
...
impl HeapState {
    ...
    fn new(ctx: &Context, cfg: &PlacerHeapCfg) -> Result<Self, PlacerError> {
    ...
    fn solve_analytical(&mut self, ctx: &Context) -> Result<(), PlacerError> {
        ...
        // Process each net.
        for net in ctx.design.net_store.iter() {
            ...
            if let Some(&idx) = self.cell_to_idx.get(&drv_cell_idx) {
                ...
                let cell = ctx.design.cell(drv_cell_idx);
                if cell.bel.is_valid() {
                    let loc = ctx.get_bel_location(cell.bel);
            ...
            for user in &net.users {
                ...
                if let Some(&idx) = self.cell_to_idx.get(&user_cell_idx) {
                    if !movable_on_net.contains(&idx) {
                        movable_on_net.push(idx);
                    }
                } else {
                    let cell = ctx.design.cell(user_cell_idx);
                    if cell.bel.is_valid() {
                        let loc = ctx.get_bel_location(cell.bel);
                        fixed_positions.push((loc.x as f64, loc.y as f64));
                    }
    ...
    fn spread(&mut self, ctx: &Context) -> Result<f64, PlacerError> {
        let n = self.movable_cells.len();
        if n == 0 {
            return Ok(1.0);
        }
        ...
        let total_bels: usize = ctx.get_bels().count();
        ...
        while let Some(region) = stack.pop() {
            ...
            if split_horizontal {
                ...
                let left_bels = count_bels_in_region(ctx, region.x0, region.y0, mid, region.y1);
                let right_bels =
                    count_bels_in_region(ctx, mid + 1, region.y0, region.x1, region.y1);
                ...
                for &idx in &left_cells {
                    self.cell_x[idx] =
                        self.cell_x[idx].clamp(region.x0 as f64, mid as f64);
                }
                for &idx in &right_cells {
                    self.cell_x[idx] =
                        self.cell_x[idx].clamp((mid + 1) as f64, region.x1 as f64);
                }
                ...
                let bottom_bels =
                    count_bels_in_region(ctx, region.x0, region.y0, region.x1, mid);
                let top_bels =
                    count_bels_in_region(ctx, region.x0, mid + 1, region.x1, region.y1);
                ...
                for &idx in &bottom_cells {
                    self.cell_y[idx] =
                        self.cell_y[idx].clamp(region.y0 as f64, mid as f64);
                }
                for &idx in &top_cells {
                    self.cell_y[idx] =
                        self.cell_y[idx].clamp((mid + 1) as f64, region.y1 as f64);
                }
    ...
    fn legalize(&mut self, ctx: &mut Context) -> Result<(), PlacerError> {
        ...
        for &idx in &order {
            ...
            for &bel in bucket_bels {
                if !ctx.is_bel_available(bel) {
                    continue;
                }
...
fn cells_by_type(ctx: &Context) -> FxHashMap<npnr_types::IdString, Vec<CellIdx>> {
    let mut map: FxHashMap<npnr_types::IdString, Vec<CellIdx>> = FxHashMap::default();
    for (i, cell) in ctx.design.cell_store.iter().enumerate() {
        if cell.alive {
            map.entry(cell.cell_type)
                .or_default()
                .push(CellIdx(i as u32));
        }
    }
    map
}
...
fn count_bels_in_region(ctx: &Context, x0: i32, y0: i32, x1: i32, y1: i32) -> usize {
    let mut count = 0;
    for bel in ctx.get_bels() {
        let loc = ctx.get_bel_location(bel);
...
mod tests {
    ...
    fn sparse_system_add_connection() {
        ...
        // Diagonal should have 5.0 at positions 0 and 2.
        assert_eq!(sys.diag[0], 5.0);
        assert_eq!(sys.diag[1], 0.0);
        assert_eq!(sys.diag[2], 5.0);

        // Off-diagonal should have one entry: (0, 2, -5.0).
    ...
    fn sparse_system_add_anchor() {
        ...
        assert_eq!(sys.rhs[0], 6.0); // 2.0 * 3.0
        assert_eq!(sys.diag[1], 0.0);
    ...
    fn sparse_system_solve_identity() {
        ...
        assert!(iters <= 3, "identity system should converge fast, got {} iters", iters);
    }

    #[test]
    fn sparse_system_solve_with_connections() {
        // Two cells connected with weight 1, cell 0 anchored at 0, cell 1 anchored at 4.
        // System:
        //   (1 + 1) * x0 - 1 * x1 = 1 * 0 = 0
        //   -1 * x0 + (1 + 1) * x1 = 1 * 4 = 4
        //   2*x0 - x1 = 0
        //   -x0 + 2*x1 = 4
        //   Solution: x0 = 4/3, x1 = 8/3
        let mut sys = SparseSystem::new(2);
        ...
        assert!(
            (x[0] - 4.0 / 3.0).abs() < 1e-6,
            "x[0] should be ~1.333, got {}",
            x[0]
        );
        assert!(
            (x[1] - 8.0 / 3.0).abs() < 1e-6,
            "x[1] should be ~2.667, got {}",
            x[1]
        );
    }
    ...
    #[test]
    fn spmv_symmetric() {
        // Verify that the result is the same as a dense symmetric multiply.
        // A = [[4, -1, 0], [-1, 4, -1], [0, -1, 4]]
        ...
        // Expected:
        // result[0] = 4*1 + (-1)*2 + 0*3 = 2
        // result[1] = (-1)*1 + 4*2 + (-1)*3 = 4
        // result[2] = 0*1 + (-1)*2 + 4*3 = 10
        assert_eq!(result[0], 2.0);
    ...
    fn spreading_cells_fit() {
        // With 4 cells and 4 BELs (2x2 grid), everything should fit.
        let ctx = make_context_with_cells(4);
        let cfg = PlacerHeapCfg::default();
        let mut state = HeapState::new(&ctx, &cfg).unwrap();

        // Place cells at various analytical positions.
        ...
        assert!(
            quality >= 0.9,
            "quality should be high when cells <= BELs, got {}",
            quality
        );
    }
    ...
    fn spreading_clustered_cells() {
        // 3 cells on 4 BELs, all initially at same position.
        let ctx = make_context_with_cells(3);
        let cfg = PlacerHeapCfg::default();
        let mut state = HeapState::new(&ctx, &cfg).unwrap();

        // All cells at the same spot.
        state.cell_x = vec![0.0, 0.0, 0.0];
        state.cell_y = vec![0.0, 0.0, 0.0];

        let quality = state.spread(&ctx).unwrap();
        // Should still report high quality since 3 cells <= 4 bels total.
        assert!(quality > 0.0);
    }
    ...
    fn count_bels_full_grid() {
        let ctx = make_context();
        let count = count_bels_in_region(&ctx, 0, 0, 1, 1);
        assert_eq!(count, 4);
    }
    ...
    fn count_bels_single_tile() {
        let ctx = make_context();
        let count = count_bels_in_region(&ctx, 0, 0, 0, 0);
        assert_eq!(count, 1);
    }
    ...
    fn count_bels_row() {
        let ctx = make_context();
        let count = count_bels_in_region(&ctx, 0, 0, 1, 0);
        assert_eq!(count, 2);
    }
    ...
    fn count_bels_empty_region() {
        let ctx = make_context();
        let count = count_bels_in_region(&ctx, 5, 5, 10, 10);
        assert_eq!(count, 0);
    }
    ...
    // =====================================================================
    // Integration tests: full HeAP run with mock context
    // =====================================================================
