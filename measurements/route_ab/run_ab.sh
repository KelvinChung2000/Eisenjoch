#!/bin/bash
# FPGA01 route A/B: the parity placement and HeAP's, through the same raster
# router under the same 90-minute cap. Sequential on purpose -- nothing else
# runs while an arm is timed.
set -u
...
# The twelve-run parity configuration, unchanged.
...
env $OT_ENV NPNR_ROUTE_PLACER=opt_trans timeout 5400 $BIN > $OUT/fpga01_ot.log 2>&1
echo "opt_trans exit=$? $(date -Is)"

echo "=== heap arm started $(date -Is) ==="
NPNR_ROUTE_PLACER=heap timeout 5400 $BIN > $OUT/fpga01_heap.log 2>&1
